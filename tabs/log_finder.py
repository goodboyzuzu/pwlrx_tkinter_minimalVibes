...
class LogFinder(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        # Columns: show formatted date (MM-DD HH:MM) instead of raw unix time
        ...
        self.tree.heading("date", text="Date", anchor="center")  # formatted date (MM-DD HH:MM)
    ...
    def _search_logs(self, event=None):
        ...
        for d in dirs:
            ...
            # Parse unix timestamp from folder name (last dot-separated token) and produce formatted date (MM-DD HH:MM)
            date_str = "-"  # default if parsing fails
            try:
                ts_part = d.split(".")[-1]
            ...
            # Insert values in the column order: offset, log_folder, date, cycle_count
